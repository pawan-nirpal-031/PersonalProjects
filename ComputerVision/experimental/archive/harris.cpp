...
computeSobelGradients(vector<vector<float>> &img) {
  ...
  vector<vector<float>> Gx(rows,vector<float>(cols,0)), Gy(rows,vector<float>(cols,0));
  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < cols; j++) {
      double gradX = 0;
      double gradY = 0;
...
vector<pair<int, int>> detectHarrisCorners(const Mat &img) {
  pair<vector<vector<float>>, vector<vector<float>>> gradients;
  int rows = img.rows;
  int cols = img.cols;
  vector<vector<float>> grayScalImg(rows,vector<float>(cols,0)), gaussFiltImg;
  for (int i = 0; i < rows; i++) {
    for (int j = 0; j < cols; j++) {
      if(img.channels()==1) 
        grayScalImg[i][j] = static_cast<int>(img.at<uchar>(i, j));
      if(img.channels()==3){
        float val = (0.0722*(img.at<Vec3b>(i,j)[0])) + (0.7152*(img.at<Vec3b>(i,j)[1])) + (0.2126*(img.at<Vec3b>(i,j)[2]));
        grayScalImg[i][j] = val;
      }
    }
  }
  gaussFiltImg = getGaussianFilterImg(grayScalImg, 3, 1);
  gradients = computeSobelGradients(gaussFiltImg);
  pair<vector<vector<float>>, float> responseMat =
      computeStrResponseMat(gradients, 0.04, 3);
...
int main() {
  ...
  if (!img.empty()) {
    ...
    vector<pair<int, int>> points = detectHarrisCorners(img);
