    ...
    void CreateBasicBlocks(){
        if(abstract_tree.size()){
            for(int line =0;line<abstract_tree.size();line++){
                ...
                if(toknized_stmt[0].back()==':')  toknized_stmt[0].pop_back();
                if(toknized_stmt.size()>=2 and toknized_stmt[toknized_stmt.size()-2]=="goto"){
            ...
            for(int line =1;line<abstract_tree.size();line++){ 
                ...
                if((label_it!=jmp_lables.end()) or (abstract_tree[line-1][0]=="if" or abstract_tree[line-1][0]=="goto")){
            ...
            if(basic_blocks.back().second!=abstract_tree.size()-1) basic_blocks.push_back({leaders.back(),abstract_tree.size()-1});  
        }
    }
...
public:
    ...
    unordered_set<string>& GetJumpLabels(){
        return jmp_lables;
    }
    ...
    void WriteTreeToOutputFile(const string &output_file="output.txt"){
        ...
        for(int i =0;i<abstract_tree.size();i++){
            ...
            fout<<i<<"  : "; 
            for(const string &token : toknized_stmt) fout<<token<<" ";
            fout<<"\n";
        }
        fout.close();
    }

    vector<pair<int,int>>& GetBasicBlocks(){
        return basic_blocks;
    }
    ...
    vector<vector<string>>& GetAbstractTree(){
