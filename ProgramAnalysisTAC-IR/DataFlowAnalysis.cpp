...
public:
  ...
  static void BlockReordering(
      vector<vector<string>> &block,
      vector<vector<pair<int, vector<int>>>> &instruction_issue_forest) {}

  static void GenerateInstructionStream(
      vector<pair<int, vector<int>>> &instruction_dep_tree,
      const set<int> inst_dependency_graph[], vector<bool> &visited, int node) {
    ...
    for (const int &child : inst_dependency_graph[node]) {
      if (not visited[child]) {
        ...
        GenerateInstructionStream(instruction_dep_tree, inst_dependency_graph,
                                  visited, child);
      }
    }
  ...
  static void GenerateInstructionIssueForest(
      const int &block_size, const set<int> inst_dependency_graph[],
      vector<vector<pair<int, vector<int>>>> &issue_forest) {
    ...
    for (int tree = 0; tree < issue_forest.size(); tree++)
      Tests::DependencyTreesTest(issue_forest[tree], tree);
  }

  static void
  ParallelIssueInstructionStreamSearch(vector<vector<string>> &block) {
    /*
        Look for parallization opportunities and independent instructions that
       can be issued in the same cycle ie improved ILP. It also enables for
       block reordering as independent instruction streams can be put togather
       improving working set size, spatial locality and imporved cache hit
       ratio. It will also allow variable Live range compression which helps in
    ...
    */
    ...
    for (int i = 0; i < block.size(); i++) {
      ...
      if (inst[1] == "=") {
        ...
        if (IsAssignment(inst)) {
          ...
          int inst_to = i;
          inst_dependency_graph[inst_from].insert(inst_to);
        } else if (inst.size() == 5 and
                   IsArithmaticOrLogicalOpreation(inst[3])) {
          int inst_to = i;
          if (oprnd_to_inst.find(inst[2]) != oprnd_to_inst.end()) {
    ...
    vector<vector<pair<int, vector<int>>>> instruction_issue_forest;
    GenerateInstructionIssueForest(block.size(), inst_dependency_graph,
                                   instruction_issue_forest);
  }
};
...
class BlockLevelAnalysis {
  ...
  void MultiplicationTransforms(vector<string> &stmt) {
    if (stmt[2] == "0" or stmt[4] == "0") {
      ...
      if (isOp1Num)
        num1 = ExtractNumber(stmt[2]);
      if (isOp2Num)
        num2 = ExtractNumber(stmt[4]);
        ...
        case (SUB): {
          if (stmt[4] == "0")
            ...
            if (num2 == "0") {
              ...
              stmt.push_back(to_string(abs(ExtractNumber(num2))));
            else
              stmt.push_back("-" + num2);
          }
        } break;
        case (MUL): { // strength reduction for powers of 2, mul with 0, mul
                      // with 1
          MultiplicationTransforms(stmt);
        } break;
        case (DIV): { // strength reduction for powers of 2, div with 0, div
                      // with 1
  ...
  void ConstantPropagation(vector<vector<string>> &block) {
    for (int i = 0; i < block.size(); i++) {
      vector<string> &stmt = block[i];
      if (stmt.size() == 3 and stmt[1] == "=" and IsANumber(stmt[2]) and
          propvalue.find(stmt[0]) == propvalue.end()) {
      ...
      if (IsArithmaticStatement(stmt)) {
        ...
        if (stmt[0] == "if") {
          if (stmt.size() == 6) { // if x op y goto L
            ...
            if (v1 != propvalue.end())
              stmt[1] = to_string(v1->second);
            if (v2 != propvalue.end())
              stmt[3] = to_string(v2->second);
  ...
  void InitVirtualTables() {
    ...
    for (int i = 0; i < opcodes.size(); i++)
      VirtualArithLogicRelnOpcodetable[opcodes[i]] = i + 1;
  }

public:
  BlockLevelAnalysis() { InitVirtualTables(); }

  void RunPassOnBlock(vector<vector<string>> &block, int passes) {
    for (int p = 0; p < passes; p++) {
      // ConstantFolding(block);
      // ConstantPropagation(block);
      InstructionLevelParallelismAnalysisAndTransforms::
          ParallelIssueInstructionStreamSearch(block);
    }
  }
};
