...
bool IsArithmaticOrLogicalOpreation(string code){
    return (code=="+" or code=="-" or code=="*" or code=="/"  or code=="%" or code=="^" or code=="|" or code=="&" 
    or code=="&&" or code=="||" or code==">>" or code=="<<" or code==">=" or code==">" or code=="<=" or code=="<" or code=="!=" or code=="==");
}
...
bool IsANumber(const string &token){ 
    for(int i =(token[0]=='-'?1:0);i<token.length();i++) if(not (token[i]>='0' and token[i]<='9')) return 0;
    return 1;
}
...
long long int ExtractNumber(string token){
    bool isneg = token[0]=='-'; 
    if(isneg) token.erase(0,1);
    long long int res =0; 
    long long int high = token.length()-1;
    for(int i =0;i<token.length();i++) res+=(token[i]-'0')*(Power(10,high-i));
...
long long int ConstantFoldingUtil(const long long int &num1,const long long int &num2,int Vopcode){
    long long int res;
