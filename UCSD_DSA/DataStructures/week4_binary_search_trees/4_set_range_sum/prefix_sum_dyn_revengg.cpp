...
public:
  ...
  void RightRotateSubtree(Node *SubtreeRoot){
    ...
    assert(LeftNode!=nullptr and "LeftNode of the subtree does not exist, Invalid Right Rotate");
  ...
  void LeftRotateSubtree(Node *SubtreeRoot){
    ...
    Node *ParentNode = SubtreeRoot->Parent; 
    assert(RightNode!=nullptr and "RightNode of the subtree does not exist, Invalid Left Rotate");
