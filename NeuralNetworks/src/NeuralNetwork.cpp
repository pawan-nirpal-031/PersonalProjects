...
void NeuralNetwork::setCurrentInput(vector<double> &Input) {
  this->Input = Input;
  assert(Layers[0]->getNumberOfNeurons() == Input.size() and
         "Input layer size not matching given input list");
...
void NeuralNetwork::printToConsole() {
  for (int i = 0; i < TopologySize; i++) {
    cout << "\n\nLayer " << i << endl;
    Matrix *m = ((i == 0) ? Layers[0]->matrixifyValues()
                          : Layers[i]->matrixifyActivatedValues());
    m->printToConsole();
    cout << "===================\n";
    if (i < Layers.size() - 1) {
...
void NeuralNetwork::setNeuronValue(int layerIndx, int neuronIndx,
                                   double Value) {
...
void NeuralNetwork::setErrors() {
  assert(Target.size() > 0 and
         "No target for this network assign or set a target.");
  assert(Target.size() == Layers[Layers.size() - 1]->getNumberOfNeurons() and
         "Target size not the same as output layer");
  Error = 0.0;
  int outputLayerIndx = Layers.size() - 1;
...
void NeuralNetwork::backpropagation() {
  ...
  int outputLayerIndx = Layers.size() - 1;
  Matrix derivedValuesHiddToOutput =
      *Layers[outputLayerIndx]->matrixifyDerivedValues();
  Matrix *gradientHiddToOutput =
      new Matrix(1, Layers[outputLayerIndx]->getNumberOfNeurons(), 0);
  ...
  Matrix *lastHiddenLayerActivated =
      Layers[lastHiddnLayerIndx]->matrixifyActivatedValues();
  Matrix *DeltaWeights = utils::multiply(gradientHiddToOutput->transpose(),
                                         lastHiddenLayerActivated)
                             ->transpose();
  Matrix *updatedWeightsHiddnToOutput =
      utils::subtract(*WeightMatrices[lastHiddnLayerIndx], *DeltaWeights);
  ...
  Matrix *currGradients = new Matrix(gradientHiddToOutput->getNumRows(),
                                     gradientHiddToOutput->getNumCols(), 0);
  ...
  for (int i = lastHiddnLayerIndx; i > 0; i--) {
    ...
    Matrix *derivedGradidents =
        new Matrix(1, CurrLayer->getNumberOfNeurons(), 0);
...
#if !Debug
        cout << "currGradients stats : " << currGradients->getNumRows() << " "
             << currGradients->getNumCols() << " # r,c " << r << " " << c
             << "\n";
#endif
        accumSum +=
            currGradients->getValue(0, c) * currWeightMatrix->getValue(r, c);
      }
      double g = accumSum * CurrLayer->getNeuronActivatedValue(r);
      derivedGradidents->setValue(0, r, g);
    }
    Matrix *leftNeurons =
        ((i == 1) ? Layers[0]->matrixifyValues()
                  : Layers[i - 1]->matrixifyActivatedValues());
    DeltaWeights = utils::multiply(derivedGradidents->transpose(), leftNeurons)
                       ->transpose();
    Matrix *NewWeightMatrixHidden =
        utils::subtract(*WeightMatrices[i - 1], *DeltaWeights);
    updatedWeights.push_back(NewWeightMatrixHidden);
    currGradients = new Matrix(derivedGradidents->getNumRows(),
                               derivedGradidents->getNumCols(), 0);
  ...
  assert(updatedWeights.size() == WeightMatrices.size() and
         "Weight matrices and updates weight matrices not equal in size");
  for (int i = 0; i < WeightMatrices.size(); i++) {
    WeightMatrices[i] = updatedWeights[WeightMatrices.size() - 1 - i];
  }
}

vector<double> getDoubleValueForMatrix(Matrix *input){
  vector<double> result; 
  for(int i =0;i<input->getNumRows();i++)
    for(int j =0;j<input->getNumCols();j++)
      result.push_back(input->getValue(i,j));
  return result;
}

void NeuralNetwork::train(vector<Matrix*> inputs,vector<Matrix*> outputs){
  for(int i=0;i<BATCH_SIZE;i++){
    vector<double> InpIntrf = getDoubleValueForMatrix(inputs[i]);
    this->setCurrentInput(InpIntrf);
    vector<double> OutputIntrf = getDoubleValueForMatrix(outputs[i]);
    this->setCurrentTarget(OutputIntrf);
    this->feedForwad();
    this->setErrors();
    cout<<this->getTotalError()<<"\n";
